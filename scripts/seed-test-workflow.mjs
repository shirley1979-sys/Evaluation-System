...
const noTeamUsers = [
  { email: 'divheadtest@everex.co.kr', name: '부문장TEST', nickname: '부문장TEST', role: 'EXECUTIVE', jobTitle: '부문장' },
]
...
async function main() {
  ...
  for (const u of noTeamUsers) {
    ...
    const { error: profileErr } = await admin.from('profiles').upsert({
      id: userId, name: u.name, nickname: u.nickname, email: u.email,
      role: u.role, team_id: null, job_title: u.jobTitle, is_active: true,
    })
    if (profileErr) throw profileErr
    console.log('profile upserted:', u.email)
  }
