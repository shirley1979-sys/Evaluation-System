...
export default function QuestionsPage() {
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) =>
      fetch(`/api/questions/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isActive }) }).then((r) => r.json()),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['questions'] }),
  })

  const deleteMutation = useMutation({
    mutationFn: (id: string) => fetch(`/api/questions/${id}`, { method: 'DELETE' }).then((r) => r.json()),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['questions'] }),
  })
      ...
      <div className="flex-1 overflow-y-auto p-7 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-sm font-semibold text-white bg-blue-600 px-4 py-2 rounded-xl hover:bg-blue-700"
          >
            + 문항 추가
          </button>
        </div>
            ...
            <tbody>
              {filtered.map((q, idx) => (
                <tr key={q.id} className={`border-t border-[#DDE3EE] hover:bg-[#F8FAFD] ${!q.isActive ? 'opacity-50' : ''}`}>
                  <td className="px-4 py-3 text-[#8896A8]">{idx + 1}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-3">
                      ...
                      <button onClick={() => { if (confirm('삭제하시겠습니까?')) deleteMutation.mutate(q.id) }} className="text-xs text-red-400 hover:underline">삭제</button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {(editTarget || newItem) && (
          <QuestionModal
            ...
            onSaved={() => { queryClient.invalidateQueries({ queryKey: ['questions'] }); setEditTarget(null); setNewItem(false) }}
          />
        )}
      </div>
    </>
...
function QuestionModal({ question, onClose, onSaved }: { question: Question | null; onClose: () => void; onSaved: () => void }) {
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) }).then((r) => r.json())
    },
    onSuccess: onSaved,
  })
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-lg p-6">
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="text-xs font-medium text-[#4A5568] mb-1 block">문항 내용</label>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[1, 3, 5].map((v) => (
              <div key={v}>
                ...
                <input value={form[`anchor${v}` as 'anchor1' | 'anchor3' | 'anchor5']} onChange={(e) => setForm({ ...form, [`anchor${v}`]: e.target.value })}
